...
const Quiz = () => {
    ...
    const integerRegex = /^(100|[1-9]?[0-9])$/;
    const [questionPointsError, setQuestionPointsError] = React.useState("");
    const [optionPercentageError, setOptionPercentageError] = React.useState("");
    ...
    const handleOptionPercentageChange = (e, index) => {
        ...
        if (!integerRegex.test(e.target.value)) {
            setOptionPercentageError("Enter a valid percentage between 0-100");
            newOptions[index].percentage = '';
        } else {
            newOptions[index].percentage = e.target.value;
            setOptionPercentageError("");
            setOptions(newOptions);
        }
    };
    ...
    const handleQuestionPointsChange = (e) => {
        ...
        if (!integerRegex.test(points)) {
            setQuestionPointsError("Enter point between 0-100");
            setQuestionPoints('');
            return;
        } else {
            setQuestionPoints(points);
            setQuestionPointsError("");
        }
    };
                ...
                <div className="create-quiz">
                    <div className="create-question">
                        ...
                        <div className="options">
                            ...
                            {questionPointsError && <p className="text-red-500">{questionPointsError}</p>}
                            ...
                            {options.map((option, index) => (
                                <div className='flex gap-5' key={index}>
                                    ...
                                    {optionPercentageError && <p className="text-red-500">{optionPercentageError}</p>}
                                </div>
                            ))}
